...
function getData(db) {
    return db.collection('cache').findOne({name: 'github'}).then(async (github) => {
        ...
        if (timeElapsed >= TWELVE_HOURS) {
            ...
            await db.collection('cache').updateOne({name: 'github'}, {
                $set: {
                    updated: new Date(),
                    issueNumbers: issueNumbers,
                    commitNumbers: commitNumbers,
                    commitNumbersWithoutTeam: commitNumbersWithoutTeam
                }
            });
        }

        return { issueNumbers: issueNumbers, commitNumbers: commitNumbers, graphCommitNumbersArray: JSON.stringify(commitNumbersWithoutTeam) };
